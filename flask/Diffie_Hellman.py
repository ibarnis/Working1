...
class DiffieHellman:
	...
	def generate_parameters(self, generator: int = 2, key_size: int = 2048) -> None:
		self.prime = self.generate_prime(key_size)
		self.generator = generator
		self.private_key = int(hexlify(urandom(32)), base=16)
	...
	def is_prime(self,n, k=128):
		...
		# Do k rounds of Miller-Rabin primality test
		for _ in range(k):
			a = randrange(2, n - 1)
			x = pow(a, d, n)
			if x == 1 or x == n - 1:
	...
	def generate_prime(self, nbits):
		"""Generate a random prime number with nbits bits."""
		while True:
			p = getrandbits(nbits)
			if self.is_prime(p):
				return p
